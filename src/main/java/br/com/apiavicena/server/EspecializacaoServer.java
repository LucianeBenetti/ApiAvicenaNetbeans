...
public class EspecializacaoServer {
    ...
    @GET
    @Produces(MediaType.APPLICATION_JSON)
    @Path("{codigoEspecializacao}")
    public EspecializacaoVO getEspecializacaoVO(@PathParam("codigoEspecializacao") Integer id) {
        EspecializacaoDAO especializacaoDAO = new EspecializacaoDAO();
    ...
    @Consumes(MediaType.APPLICATION_FORM_URLENCODED)
    @Path("{codigoEspecializacao}")
    public Boolean alterar(@PathParam("codigoEspecializacao") Integer id, @FormParam("dado") String dadosJSON) {
        Gson gson = new Gson();
        EspecializacaoVO especializacaoVO = gson.fromJson(dadosJSON, EspecializacaoVO.class);

        if (id == especializacaoVO.getCodigoEspecializacao() || especializacaoVO.getCodigoEspecializacao() == null ) {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Path("{codigoEspecializacao}")
    public Boolean deletar(@PathParam("codigoEspecializacao") Integer id) {
        EspecializacaoDAO especializacaoDAO = new EspecializacaoDAO();
