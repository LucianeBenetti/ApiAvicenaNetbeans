...
public class PacienteServer {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Path("{codigoPaciente}")
    public PacienteVO getPacienteVO(@PathParam("codigoPaciente") int id) {
        PacienteDAO pacienteDao = new PacienteDAO();
    ...
    @Consumes(MediaType.APPLICATION_FORM_URLENCODED)
    @Path("{codigoPaciente}")
    public Boolean alterar(@PathParam("codigoPaciente") int id, @FormParam("dado") String dadosJSON) {
        Gson gson = new Gson();
       PacienteVO pacienteVO = gson.fromJson(dadosJSON, PacienteVO.class);

        if (id == pacienteVO.getCodigoPaciente() || pacienteVO.getCodigoPaciente() == null) {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Path("{codigoPaciente}")
    public Boolean deletar(@PathParam("codigoPaciente") Integer codigoPaciente) {
        PacienteDAO pacienteDao = new PacienteDAO();
        Boolean res = pacienteDao.excluir(codigoPaciente);
        Conexao.closeConnection();
        return res;
    }
}
